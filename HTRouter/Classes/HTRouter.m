...
+ (HTRouter *)shared {
    static HTRouter *_shared;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _shared = [HTRouter new];
    });
    return _shared;
}
...
+ (void)startupWithHomeViewController:(NSString *)viewControllerName {
    ...
    [[HTRouter shared] setHomeViewController:viewControllerName];
    window.rootViewController = [HTRouter shared].rootViewController;
...
- (void)presentDialog:(id<HTRouterViewControllerDialog>)dialog completeBlock:(DXRouterDialogCompleteBlock)completeBlock {
...
- (UIViewController *)viewControllerInstanceWithName:(NSString *)name {
    ...
    if (cls && [cls conformsToProtocol:@protocol(HTRouterViewControllerInstantiation)]) {
        UIViewController<HTRouterViewControllerInstantiation> *viewController = [cls instantiateViewController];
        return viewController;
    }
    return [HTNotExistViewController new];
}
